...
async function main() {
  ...
  const dbName = process.env.MONGODB_LOCAL_DBNAME || 'creative_workbench'
  if (!uri) throw new Error('缺少 MONGODB_LOCAL_URI')
...
main().catch(err => { console.error('list-indexes-local 失败：', err); process.exit(1) })